...
export class AddressBook implements Middleware<Context> {
    ...
    async fn(context: Context, next: () => Promise<void>) {
        ...
        const computedPayload = await getSendTransactionComputedPayload(context.request)
        if (!computedPayload) return

        const from = this.getFrom(computedPayload)
        const to = this.getTo(computedPayload)

        if (!isSameAddress(from, to) && !isZeroAddress(to)) await WalletRPC.addAddress(context.chainId, to)
    }
}
